...
public:
    ...
    void run(int in) {
        ...
        computer.add_input(in);
        computer.run(true, &output);
    }

    long long int get_output() {
        return output;
    }
...
int main() {
    ...
    do {
        ...
        for(int i = 0; i < 5; ++i) {
            std::vector<long long int> input{ phase_settings[i], output };

            computer.reset();
            computer.add_input({ phase_settings[i], output });
            computer.run(false, &output);
